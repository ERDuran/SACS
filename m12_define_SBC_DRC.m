...
[ALLC_lat_v_north, ALLC_lat_v_south] = deal(NaN(1, length(lon_u_ALLC)-1));
nc = 0;
for jj = 1 : length(lon_v)
    ...
    if ~isempty(ALLC_lat_v_north_now)
        nc = nc + 1;
        ALLC_lat_v_north(nc) = lat_v(ALLC_lat_v_north_now);
        ALLC_lat_v_south(nc) = lat_v(ALLC_lat_v_south_now);
    end
end
...
lon_min = 114; lon_max = 148; lat_min = -46; lat_max = -31.5;
