...
for kk = 1 : length(pres)
    v_interp2_OFAM(:,:,kk) = interp2(lon_OFAM, lat_OFAM, ...
        v_OFAM(:,:,kk), lon_v_CARS, lat_v_CARS);
end

v_OFAM_minus_CARS = v_interp2_OFAM - v_CARS;
...
lon_now = lon_v_CARS(lon_now_ind);
v_OFAM_now = squeeze(v_interp2_OFAM(:,lon_now_ind,:))';
v_OFAM_minus_CARS_now = squeeze(v_OFAM_minus_CARS(:,lon_now_ind,:))';

% 3) asal pcolor set-up
v_OFAM_minus_CARS_contours = -0.225:0.025:0.225;
v_OFAM_minus_CARS_contours_tick = v_OFAM_minus_CARS_contours(2:end-1);
cmap_levels = length(v_OFAM_minus_CARS_contours);
...
pcolor(...
    lat_v_CARS, ...
    pres, ...
    v_OFAM_minus_CARS_now)
shading interp
axis ij
hold on
caxis([v_OFAM_minus_CARS_contours(1) v_OFAM_minus_CARS_contours(end)]);
axis([-39.5 -34.5 0 2000])
cbar = colorbar;
set(cbar, 'visible', 'off');

% 5) Theta contours set-up
v_OFAM_pos_contours = 0.05:0.05:0.5;
v_OFAM_neg_contours = -0.05:-0.05:-0.5;
v_OFAM_zero_contours = [0 0];
...
axes(h_axes_sp(sp))
lon_now = 147;
lon_now_ind = find(lon_u_CARS == lon_now)-1;
lon_now = lon_v_CARS(lon_now_ind);
v_OFAM_now = squeeze(v_interp2_OFAM(:,lon_now_ind,:))';
v_OFAM_minus_CARS_now = squeeze(v_OFAM_minus_CARS(:,lon_now_ind,:))';
colormap(h_axes_sp(sp), cmap);
...
cbarrow
set(cbar, 'YTick', ...
    v_OFAM_minus_CARS_contours_tick(...
    v_OFAM_minus_CARS_contours_tick~=0));
