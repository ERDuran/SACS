...
fig_n = 1;
rowcols = [4 2];
rowcols_size = [7 5]/screen_ratio; % cm
...
magnif = 100;
cmap1_cont = -[14 12 10 8 6 4 2 1 0];
cmap2_cont = [0 1 2 4 6 8 10 12 14];
...
for sp = 1 : rowcols(1)*rowcols(2)
    ...
    if sp == 5
        axis_setup{sp} = [...
            aus8_coor.pi180 * aus8_coor.a * (aus8_figures.cross.lat(2,sp)+1.5), ...
            aus8_coor.pi180 * aus8_coor.a * (aus8_figures.cross.lat(1,sp)-1), ...
            -800, ...
            0];
    else
        axis_setup{sp} = [...
            aus8_coor.pi180 * aus8_coor.a * (aus8_figures.cross.lat(2,sp)+2.5), ...
            aus8_coor.pi180 * aus8_coor.a * aus8_figures.cross.lat(1,sp), ...
            -800, ...
            0];
    end
    ...
    [x_grid, y_grid] = meshgrid(x_contour_2{sp}, y_contour_2{sp});
    [x_grid_hr, y_grid_hr] = meshgrid(x_contour{sp}, y{sp});
    pcolor(x_grid, y_grid, data_contour_2{sp})
    data_contour_2_hr{sp} = interp2(...
        x_grid, y_grid, data_contour_2{sp}, ...
        x_grid_hr, y_grid_hr);
end
...
for sp = 1 : rowN*colN
    ...
    x_meter = aus8_coor.pi180 * aus8_coor.a * x{sp};
    x_contour_meter = aus8_coor.pi180 * aus8_coor.a * x_contour{sp};
    
    colormap(ax, cmaps_custom{sp});
    pcolor(x_meter, y{sp}, data{sp})
    axis(axis_setup{sp})
    shading interp
...
%     quiver(...
%         -3.95*10^6, -200, ...
%         aus8_coor.a*aus8_coor.pi180, 100, ...
%         'k', 'autoscale', 'off')
%     grid
%     set(ax,'layer','top')
%     stop 
    ...
    quiver(...
        x_contour_meter(:), ...
        y{sp}([start:mid:finish,finish+mid:end]), ...
        5000*data_contour{sp}([start:mid:finish,finish+mid:end],:), ...
        5000*data_contour_2_hr{sp}([start:mid:finish,finish+mid:end],:), ...
        'autoscale', 'off', ...
        'maxheadsize', 0.0001, 'color', 'k')
...
%         'linewidth', 0.9);
    ...
    if find(ismember(lon_u_SBC_north_OK,aus8_figures.cross.lon(1,sp)))
        plot([...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_SBC_north(...
    ...
    if aus8_figures.cross.lon(1,sp)~=147
        plot([...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_SBC_south(...
            aus8_currents.lon_u_ALLC==aus8_figures.cross.lon(1,sp)),...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_SBC_north(...
            aus8_currents.lon_u_ALLC==aus8_figures.cross.lon(1,sp))],...
        ...
        plot([...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_SBC_south(end),...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_SBC_north(end)],...
            [aus8_currents.z_mid, aus8_currents.z_mid] ...
            , 'b', ...
            'linewidth',line_width)
    end
    ...
    if aus8_figures.cross.lon(1,sp)~=147
        plot([...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_DRC_south(...
            aus8_currents.lon_u_ALLC==aus8_figures.cross.lon(1,sp)),...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_DRC_south(...
            aus8_currents.lon_u_ALLC==aus8_figures.cross.lon(1,sp))],...
            [aus8_currents.z_mid, aus8_currents.z_top] ...
        ...
        plot([...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_DRC_south(end),...
            aus8_coor.pi180 * aus8_coor.a * aus8_currents.lat_v_DRC_south(end)],...
            [aus8_currents.z_mid, aus8_currents.z_top] ...
            , ...
            '-m', ...
...
% 
    ...
    set(ax,'layer','top','color',nan_color,...
        ...
        'xtick', ...
        axis_setup{sp}(1):...
        aus8_coor.pi180*aus8_coor.a/2:...
        axis_setup{sp}(2)-1,...
        'xticklabel', ...
        (axis_setup{sp}(1):...
        aus8_coor.pi180*aus8_coor.a/2:...
        axis_setup{sp}(2)-1)/aus8_coor.pi180/aus8_coor.a)
    if col_ind(sp) ~= 1
        set(gca,'yticklabel','')
    else
        ylabel('Depth (m)')
