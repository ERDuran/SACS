...
SC_bottom_pres_mid_ind = find(pres_mid == SC_bottom_pres_mid);
...
%% 2) Define SBC region and set north boundary
%
...
for kk = SC_pres_mid_length-1:-1:1
    ...
    for jj = ALLC_west_to_east_lon_u_ind
        ...
        % lat u
        SBC_u_g_prime_now = u_g_prime(:,jj,kk);
        
        if jj >= 57 && jj <= 65
            SBC_u_g_prime_now(1:32) = NaN;
        end
        ...
        elseif jj > jj_1 && jj <= jj_2
            p_now = 95; p_now_ind = find(pres_mid == p_now)+1;
            if pres_mid(kk) <= p_now
                flag = 1;
                mask_ind = SC_v(:,jj,p_now_ind) == 0;
                SBC_u_g_prime_now(mask_ind) = NaN;
            end
            
        elseif jj > jj_2
            SBC_u_g_prime_now(1:88) = NaN;
            
        end
        
        SC_lat_u_first_ocean_now_ind = ...
            find(isfinite(SBC_u_g_prime_now), 1, 'first');
...
[SBC_lat_v_north, SC_region_lat_v_south] = ...
...
for kk = 1 : length(pres_mid)
    ...
    for jj = ALLC_west_to_east_lon_u_ind
        ...
        if ~isempty(SC_lat_v_north_ind_now)
            SBC_lat_v_north(kk, jj_count) = ...
                SC_lat_v_all(SC_lat_v_north_ind_now, jj, kk);
            SC_region_lat_v_south(kk, jj_count) = ...
...
SBC_lon_u_repelem = [...
    SCB_lon_u(:,1), ...
    repelem(SCB_lon_u(:,2:end-1), 1, 2), ...
    SCB_lon_u(:,end)];
SBC_lat_v_north_repelem = ...
    repelem(SBC_lat_v_north, 1, 2);
SC_region_lat_v_south_repelem = ...
    repelem(SC_region_lat_v_south, 1, 2);


%% 3) Set south boundary
...
U_g_prime_0_to_pmid = ...
    nansum(u_g_prime_times_depth_h_u(:,:,SC_bottom_all_pres_mid_ind), 3);
U_0_to_pmid = U_g_prime_0_to_pmid + U_ek;

v_g_prime_times_depth_h_v = v_g_prime .* depth_h_v;
V_g_prime_0_to_pmid = ...
    nansum(v_g_prime_times_depth_h_v(:,:,SC_bottom_all_pres_mid_ind), 3);
V_0_to_pmid = V_g_prime_0_to_pmid + V_ek;

U_g_prime_pmid_to_pb = ...
    nansum(u_g_prime_times_depth_h_u(:,:,~SC_bottom_all_pres_mid_ind), 3);

V_g_prime_pmid_to_pb = ...
    nansum(v_g_prime_times_depth_h_v(:,:,~SC_bottom_all_pres_mid_ind), 3);


SBC_lat_v_south = NaN(size(SC_region_lat_v_south));

jj_count = 0;
for jj = ALLC_west_to_east_lon_u_ind(1:end-1)
    jj_count = jj_count + 1;
    SBC_section_now = squeeze(SC_u(:,jj,:))';
    SBC_section_now_max = max(max(SBC_section_now));
    [SBC_section_now_max_row_ind, SBC_section_now_max_col_ind] = ...
        find(SBC_section_now == SBC_section_now_max);
    
    southward_line_from_max_ind = ...
        SBC_section_now_max_col_ind:length(lat_u);
    
    section_now = squeeze(u_g_prime(:,jj,:))';
    southward_line_from_max = section_now(...
        SBC_section_now_max_row_ind, southward_line_from_max_ind);
    southward_line_last_SBC_ind = find(southward_line_from_max < 0, ...
        1, 'first') -1;
    
    if southward_line_last_SBC_ind < 3
        southward_line_last_SBC_ind = 3;
    end
    
    
    SBC_lat_v_south(SC_bottom_all_pres_mid_ind, jj_count) = ...
        repmat(lat_v(...
        southward_line_from_max_ind(southward_line_last_SBC_ind) +1), ...
        SC_bottom_pres_mid_ind, 1);
end
SBC_lat_v_south_repelem = ...
    repelem(SBC_lat_v_south, 1, 2);


%% 4) Get SBC

SBC_u_g_prime = NaN(size(u_g_prime));
SBC_v_g_prime = NaN(size(v_g_prime));
%%% GET U AND V
for kk = SC_pres_mid_length-1:-1:1
    % longitude index counter
    jj_count = 0;
    ...
    for jj = ALLC_west_to_east_lon_u_ind(1:end-1)
        ...
        % get lat u SBC
        lat_u_SBC_now_ind = ismember(lat_u, ...
            SBC_lat_v_north(kk,jj_count)-1/16 : -1/8 : ...
            SBC_lat_v_south(kk,jj_count)+1/16);
        lat_v_SBC_now_ind = ismember(lat_v, ...
            SBC_lat_v_north(kk,jj_count) : -1/8 : ...
            SBC_lat_v_south(kk,jj_count));
...
SBC_u_g_prime_ind = SBC_u_g_prime ~= 0;
SBC_v_g_prime_ind = SBC_v_g_prime ~= 0;


%%
%%% SAVE
aus8_currents.SBC.u_g_prime_ind = SC_u_ind;
aus8_currents.SBC.v_g_prime_ind = SC_v_ind;
aus8_currents.SBC.lon_u = SCB_lon_u;
aus8_currents.SBC.lat_v_north = SBC_lat_v_north;
aus8_currents.SBC.lat_v_south = SBC_lat_v_south;
aus8_currents.SBC.lon_u_repelem = SBC_lon_u_repelem;
aus8_currents.SBC.lat_v_north_repelem = SBC_lat_v_north_repelem;
aus8_currents.SBC.lat_v_south_repelem = SBC_lat_v_south_repelem;
%%% SAVE
...
%
data.SBC_u_g_prime = SBC_u_g_prime(lat_u_ind,lon_u_ind,p_ind);
data.SBC_v_g_prime = SBC_v_g_prime(lat_v_ind,lon_v_ind,p_ind);
...
hold on
% west
plot([SCB_lon_u(1,1) SCB_lon_u(1,1)], ...
    [SBC_lat_v_north(p_ind,1) SBC_lat_v_south(p_ind,1)], ...
    'g','linewidth',0.5,'linestyle','-')
% north
plot(SBC_lon_u_repelem(p_ind,:), ...
    SBC_lat_v_north_repelem(p_ind,:), ...
    'g','linewidth',0.5,'linestyle','-')
% south
plot(SBC_lon_u_repelem(p_ind,:), ...
    SBC_lat_v_south_repelem(p_ind,:), ...
    'g','linewidth',0.5,'linestyle','-')
% east
plot([SCB_lon_u(1,end) SCB_lon_u(1,end)], ...
    [SBC_lat_v_north(p_ind,end) SBC_lat_v_south(p_ind,end)], ...
    'g','linewidth',0.5,'linestyle','-')
...
nn = 1;
quiv_S = 5;
SBC_U_g_times_depth = SBC_u_g_prime.*depth_h_u;
