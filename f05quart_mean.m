...
salt_mean_KDau_temp = KDau_salt.mean(:,:,N:N+1);
salt_mean_aus8_temp = aus8_salt.mean(:,:,N:N+1);
...
u_g_mean_aus8 = aus8_u_g_prime.mean(:,:,N);
v_g_mean_aus8 = aus8_v_g_prime.mean(:,:,N);
...
data = ...
    {interp2(lon_u,lat_u,KDau_fulu.mean(:,:,N),lon_v,lat_v)*magnif, ...
    interp2(lon_u,lat_u,u_g_mean_aus8(:,:,1),lon_v,lat_v)*magnif};
v_data = {KDau_fulv.mean(:,:,N)*magnif, ...
    v_g_mean_aus8(:,:,1)*magnif};
...
%     cbar_contour = linspace(34.40,34.60,16);
    cbar_contour = linspace(34.20,34.70,31);
    ...
    contourf(lon,lat,data_pcolor{DD},cbar_contour,'linewidth',0.1)
    axis(axis_setup)
    shading interp
    get_caxis = caxis;
